...
from src.database import create_database_engine, fetch_stock_data
from src.dataset import split_features_target, prepare_training_dataset
from src.feature_engineering import engineer_features
from src.market_features import add_market_features
from src.model_training import DEFAULT_XGBOOST_MODEL_PATH, load_xgboost_model
...
def build_feature_dataset(stock_data: pd.DataFrame) -> pd.DataFrame:
    ...
    end_date = featured_data["date"].max()
...
def get_model_feature_columns(featured_data: pd.DataFrame) -> list[str]:
    ...
    if "log_future_volatility" in featured_data.columns:
        ...
        feature_source = featured_data.assign(log_future_volatility=0.0)

    features, _ = split_features_target(feature_source)
    return list(features.columns)


def calculate_production_risk_thresholds(featured_data: pd.DataFrame) -> tuple[float, float]:
    """Calculate risk thresholds with the same pre-2024 target window as training."""

    training_data = create_future_volatility_target(featured_data)
    training_data = remove_missing_targets(training_data)
    training_data = create_log_volatility_target(training_data)
    training_data = prepare_training_dataset(training_data)

    threshold_data = training_data[
        training_data["date"] < pd.Timestamp(TRAIN_END_DATE)
    ]

    if threshold_data.empty:
        raise ValueError("No training-period target data is available for risk thresholds.")

    return calculate_risk_thresholds(threshold_data["future_volatility"])
...
def generate_latest_predictions(
    ...
    if latest_rows.empty:
        raise ValueError("No complete latest feature rows are available for prediction.")

    low_threshold, high_threshold = calculate_production_risk_thresholds(featured_data)
    ...
    result = latest_rows[["ticker", "date"]].copy()
    result["predicted_volatility"] = predictions.to_numpy()
    result["risk"] = risk_labels.astype(str).to_numpy()
